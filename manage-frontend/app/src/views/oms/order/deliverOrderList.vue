...
<script>
import { deliveryOrder, getDeliveryList } from "@/api/order";
...
export default {
  ...
  created() {
    ...
    this.getDefaultLogisticsCompanies();
  },
  methods: {
    async getDefaultLogisticsCompanies() {
    ...
    confirm() {
      if (!this.list[0].deliveryCompany) {
        this.$message({
          type: "error",
          message: "请选择物流公司"
        });
