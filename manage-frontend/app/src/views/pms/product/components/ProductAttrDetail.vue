  ...
  <div style="margin-top: 50px">
    ...
    >
          ...
          <el-table-column label="库存预警值" width="80" align="center">
            <template slot-scope="scope">
              <el-input
                ...
                @change="watchLowStock(scope.$index,$event,scope.row.stock)"
              ></el-input>
            </template>
          </el-table-column>
      ...
      <el-form-item label="商品相册：">
        <multi-upload v-model="value.albumPics"></multi-upload>
      </el-form-item>
      <el-form-item label="规格参数：">
        <el-tabs v-model="activeHtmlName" type="card">
          <el-tab-pane label="电脑端详情" name="pc">
            <tinymce :isView="false" :width="595" :height="300" v-model="value.detailHtml"></tinymce>
          </el-tab-pane>
      ...
      <el-form-item style="text-align: center">
        ...
        <el-button type="primary" size="medium" @click="handleNext('productAttrForm')">完成，提交商品</el-button>
...
export default {
  ...
  methods: {
    watchLowStock(index, event, stock) {
      console.log(1);
      if (event - stock >= 0) {
        this.$set(this.value.skuStockList[index], "lowStock", stock);
        console.log(this.value.skuStockList[index], stock);
      }
    },
    handleEditCreated() {
      // 根据商品属性分类id获取属性和参数
    ...
    handleNext(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (!this.value.skuStockList.length) {
            this.$message({
              message: "请刷新sku列表",
              type: "error",
              duration: 1000
          ...
          for (let i = 0; i < this.value.skuStockList.length; i++) {
            if (
              !this.value.skuStockList[i].price ||
              !this.value.skuStockList[i].stock ||
              !this.value.skuStockList[i].lowStock ||
              !this.value.skuStockList[i].skuCode
            ) {
              this.$message({
                message:
                  "请完善sku列表                                                                                                           ",
    ...
    getPic() {
      if (this.value.albumPics) {
        this.value.pic = JSON.parse(this.value.albumPics)[0];
      }
    }
  }
};
